...
export function channelPublicView(ch) {
  ...
  return {
    ...
    blog_articles_count: Array.isArray(ch.blog_articles) ? ch.blog_articles.length : 0,
    blog_articles_updated_at: ch.blog_articles_updated_at || null,
    ...
    settings: { daily_publish_time: ch.daily_publish_time, target_duration_sec: ch.target_duration_sec,
      ...
      publish_schedule_mode: ch.publish_schedule_mode || ((Array.isArray(ch.publish_times) && ch.publish_times.length) ? 'fixed' : 'auto'),
